...
public class HUD {
    ...
    private static List<Polyline> connectors;
    ...
    private static void drawConnectors(List<Double> smallShapePoints, List<Double> bigShapePoints, Color color) {
        ...
        for (int i = 0; i < bigShape.size(); i++) {
            if (Math.sqrt(Math.pow(bigShape.get(i).getPoints().get(2) - bigShape.get(i).getPoints().get(0), 2) +
                    Math.pow(bigShape.get(i).getPoints().get(3) - bigShape.get(i).getPoints().get(1), 2)) != bigSideLength) {
                isRegular = false;
                break;
            }
        }
